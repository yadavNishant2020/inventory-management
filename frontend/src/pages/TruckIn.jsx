...
function TruckIn() {
  ...
  const handleSubmitAll = async () => {
    ...
    try {
      ...
      showToast(`Truck IN: Added ${itemsToAdd.length} item(s) to stock`, 'success')
  ...
  const totalItemsToAdd = Object.values(quantities).filter(q => parseInt(q) > 0).length
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs">Truck arrived - add stock</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <DatePicker
              ...
              className="w-full px-3 py-2.5 rounded-lg border border-slate-200 text-sm font-semibold text-slate-800 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
        ...
        <input
          ...
          placeholder="Remark (e.g., Truck MP04G8)"
          className="w-full px-3 py-2.5 rounded-lg border border-slate-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
        />
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {/* Header */}
        <div className="grid grid-cols-[1fr,80px] bg-slate-50 border-b border-slate-200 px-4 py-2.5">
          <span className="text-xs font-semibold text-slate-600 uppercase">Name</span>
          <span className="text-xs font-semibold text-slate-600 uppercase text-center">Qty</span>
        </div>

        {/* Items */}
        {items.length === 0 ? (
          <div className="p-8 text-center">
            <p className="text-slate-500 text-sm">No items in database.</p>
            <Link to="/add-item" className="text-primary-600 text-sm font-medium">Add items first</Link>
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            {items.map(item => (
              <div key={item.id} className="grid grid-cols-[1fr,80px] items-center px-4 py-3">
                <div>
                ...
                <input
                  ...
                  className="w-full px-2 py-2 rounded-lg border border-slate-200 text-center font-semibold text-emerald-600 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
                />
              </div>
            ))}
          </div>
      ...
      {items.length > 0 && (
        <div className="fixed bottom-20 lg:bottom-6 left-4 right-4 lg:left-auto lg:right-6 lg:w-96">
          <button
            ...
            disabled={submitting || totalItemsToAdd === 0}
          ...
          >
            {submitting ? (
              ...
              <>
                ...
                Truck IN ({totalItemsToAdd} items, +{totalQuantity})
              </>
            )}
          </button>
        </div>
