...
function Navbar() {
  ...
  // Detect if we're in the crates section
  const isCratesSection = location.pathname.startsWith('/crates')
  ...
  // Truck navigation links (default)
  const truckNavLinks = [
  ...
  // Crates navigation links
  const cratesNavLinks = [
    { to: '/crates', label: 'Home', icon: CrateIcon, key: 'crates-home' },
    { to: '/crates/customers', label: 'Customers', icon: UsersIcon, key: 'crates-customers' },
    { to: '/crates/entry', label: 'Entry', icon: PlusIcon, key: 'crates-entry' },
    { to: '/crates/customers', label: 'Ledger', icon: LedgerIcon, key: 'crates-ledger' },
  ]

  // Use appropriate nav links based on current section
  const navLinks = isCratesSection ? cratesNavLinks : truckNavLinks
            ...
            <div className="flex items-center gap-1">
              {navLinks.map((link) => (
                ...
                >
                  ...
                  {isCratesSection ? (
                    // Crates section labels
                    link.label === 'Home' ? 'Dashboard' :
                    link.label === 'Customers' ? 'Customers' :
                    link.label === 'Entry' ? 'Add Entry' :
                    link.label === 'Ledger' ? 'View Ledger' : link.label
                  ) : (
                    // Trucks section labels
                    link.label === 'Home' ? 'Dashboard' : 
